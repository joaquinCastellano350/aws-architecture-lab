...
export const checkoutApiVersion = "6.0.0";
...
export interface CheckoutStatusResponse {
  readonly checkoutId: string;
  readonly status: "PENDING" | "INVENTORY_UNAVAILABLE" | "EXPIRED" | "COMPENSATING" | "RECONCILIATION_REQUIRED" | "CANCELLED" | "CONFIRMED";
  readonly correlationId: string;
...
export function validateCheckoutStatusResponse(
  ...
  if (
    ...
    input === null ||
    (typeof (input as Record<string, unknown>)["checkoutId"] !== "string") ||
    (typeof (input as Record<string, unknown>)["status"] !== "string" || !["PENDING", "INVENTORY_UNAVAILABLE", "EXPIRED", "COMPENSATING", "RECONCILIATION_REQUIRED", "CANCELLED", "CONFIRMED"].includes((input as Record<string, unknown>)["status"] as string)) ||
