...
const definitions = await Promise.all([
  load("reconciliation-required-event.v1.json", "validateReconciliationRequiredEvent"),
  load("reconciliation-resolved-event.v1.json", "validateReconciliationResolvedEvent"),
]);
const lines = ["// Generated from schemas/reconciliation-*-event.v1.json. Do not edit by hand.", ""];
...
for (const { schema, validatorName } of definitions) {
  lines.push(...objectValidatorLines({
    ...
    resultType: "ReconciliationEventValidationResult",
  }));
}
await writeFile(new URL("../src/generated/reconciliation-event.ts", import.meta.url), `${lines.join("\n")}\n`);
